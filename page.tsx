...
import { trackInitiateCheckout, trackPurchase } from "@/lib/checkoutTracking";
...
export default function CheckoutPage() {
    ...
    pollRef.current = setInterval(async () => {
      try {
        ...
        if (data.status === "COMPLETED") {
          if (pollRef.current) clearInterval(pollRef.current);
          if (timerRef.current) clearInterval(timerRef.current);
          trackPurchase({ transactionId: correlationID, valueCents: totalCents });
  ...
  const handleGeneratePIX = async () => {
    ...
    try {
      ...
      trackInitiateCheckout(totalCents);
      startTimer();
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 10 }}>
            {INCLUDED_BONUSES.map((bonus) => (
              <div key={bonus.title} style={{ minWidth: 0, textAlign: "center" }}>
                <img src={bonus.image} alt={bonus.title} style={{ width: "100%", height: 290, objectFit: "contain", display: "block", borderRadius: 8 }} />
                <div style={{ color: "#166534", fontSize: 12, fontWeight: 700, lineHeight: 1.35, marginTop: 6 }}>{bonus.title}</div>
              </div>
            ))}
          </div>
        </div>
              ...
              {!pixData ? (
                ...
                <>
                  ...
                  <div style={S.qrWrap}>
                    ...
                    >
                      Enviamos o PIX para{" "}
                      <strong style={{ color: "#15803d" }}>{email}</strong>. Confira
