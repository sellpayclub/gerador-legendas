...
async def create_checkout_charge(body: CreateChargeBody) -> dict:
    ...
    charge_result = create_pix_charge(
    ...
    items_payload = [item.dict() for item in body.items]
    email_result = send_pix_pending_email(
        to_email=body.email.strip().lower(),
        customer_name=body.name,
        total_cents=body.total_cents,
        items=items_payload,
        br_code=str(charge_result.get("brCode") or ""),
        qr_code_image_url=str(charge_result.get("qrCodeImage") or ""),
        payment_link_url=str(charge_result.get("paymentLinkUrl") or ""),
        correlation_id=correlation_id,
        expires_minutes=5,
    )
    pix_email_sent = bool(email_result.get("ok"))
    if not pix_email_sent:
        log.warning(
            "PIX email failed for %s: %s",
            correlation_id,
        ...
        try:
            rest_patch(
                ...
                body={
                    "pix_email_sent_at": datetime.now(timezone.utc).isoformat(),
                    "pix_email_id": str(email_result.get("email_id") or "") or None,
                },
            )
        except Exception as exc:
            log.warning("Failed to record pix email for %s: %s", correlation_id, exc)
